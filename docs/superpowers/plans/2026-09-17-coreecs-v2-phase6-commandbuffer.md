...
```
规则要点：kind 按最派生接口判定（Tag > Discrete > Dense）；只有 Dense 与实体掩码决定 Structure 归属，Discrete / Tag 不迁移；三类都会调用 `OnCreate` / `OnDestroy`（Tag 用默认空实现）；`GetComponent<Tag>` 返回 `default`（`NotNull == false`）。
