...
15. **文档评审记录（Task 4）**：独立文档评审结论「文档评审通过」，无 Critical/Important。六个覆盖点（CommandBuffer / `IEntityQuery` / `s.RO/RW<T>()` / 系统分组排序 / World 生命周期钩子 / 破坏性变更）在中英文四份文档中均有源码核对过的覆盖；v1 名称仅出现在破坏性变更清单；中英结构/代码块一一对应；tag 生命周期表述与内核一致。Minor 记录（不阻塞）：QUICK_START 与 README 的「在 `OnRegister` 中注册依赖」宜明确为 `World.OnRegister`（`ISystem` 无该成员）；tag 增删除翻转位图外还会发 collector 事件（文档只提位图）；占位实体「之后引用抛异常」仅对 buffer 命令成立（读 `EntityId` 不抛）；README 中英文文档表行数不对称（中文少一行英文 README）。
16. **文档修订（Task 4，文档评审前）**：实现者发现 Task 4 Step 3(c) 的「三类都会调用 `OnCreate` / `OnDestroy`（Tag 用默认空实现）」与内核实际行为不符——`AddTagComponent` / `RemoveTagComponent` 只翻转位图，不调用 `ComponentHookDispatcher`。该偏差是 Plan 1b/1c 的有意决定（Plan 1c Self-Review 第 10 条：spec §2.1 要求三类均调用 hook，内核有意跳过 Tag；Tag 默认空实现，行为等价；若未来需要 Tag 自定义 hook 需在内核补 `InvokeTagCreate/InvokeTagDestroy`）。文档必须描述真实行为，故计划改为「Dense / Discrete 在增删时调用 `OnCreate` / `OnDestroy`，Tag 使用默认空实现且内核不调用 tag hook」，并在文档修订提交中同步中英文。
